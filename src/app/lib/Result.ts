export class Result<TValue, TError = string> {
    ...
    public constructor(isSuccess: boolean, error: TError | null, value: TValue | null) {
      if (isSuccess && error) {
        console.error('InvalidOperation: A result cannot be successful and contain an error')
        this.printError()
      }
      if (!isSuccess && !error) {
        console.error('InvalidOperation: A failing result needs to contain an error message')
        this.printError()
      }
    ...
    public value(): TValue {
      if (!this.isSuccess) {
        console.error('Can not get the value of an error result. Use errorValue instead.')
    ...
    public static combine<T = unknown, F = unknown>(results: Result<T, F>[]): Result<T, F> {
